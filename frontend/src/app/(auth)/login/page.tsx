...
export default function LoginPage() {
    ...
    const handleInitialLogin = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:5000"}/api/auth/login`, {
                email,
                password,
            });
            setStep(2);
    ...
    const handleVerifyLogin = async (e: React.FormEvent) => {
        ...
        try {
            const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:5000"}/api/auth/login/verify`, {
                email,
                otp
            });
                ...
                {step === 1 ? (
                    <form onSubmit={handleInitialLogin} className="space-y-6">
                        ...
                        <div className="text-right -mt-2">
                            <Link href="/forgot-password" className="text-xs font-semibold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500 hover:opacity-80 transition-opacity">
                                Forgot Password?
                            </Link>
                        </div>
                        <button
